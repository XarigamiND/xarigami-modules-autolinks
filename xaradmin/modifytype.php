...
function autolinks_admin_modifytype($args)
{
    ...
    if (!empty($submit)) {
        ...
        if (!xarVarFetch('type_name', 'pre:lower:passthru:str:1', $type['type_name'])) {
            $errorcount += 1;
            $type['type_name_error'] = xarML("No 'type_name' provided");
        }

        if (!xarVarFetch('template_name', 'pre:ftoken:lower:passthru:str:1', $type['template_name'])) {
            $errorcount += 1;
            $type['template_name_error'] = xarML("No 'template_name' provided");
        }

        if (!xarVarFetch('dynamic_replace', 'int:0:1', $type['dynamic_replace'], '0')) {
            $errorcount += 1;
            $type['dynamic_replace_error'] =xarML("No 'dynamic_replace' provided");
        }

        if (!xarVarFetch('type_desc', 'str:0:400', $type['type_desc'])) {
            $errorcount += 1;
            $type['type_desc_error'] = xarML("No 'type_desc' provided");
        }
        ...
        if ($errorcount == 0) {
            ...
            if ($result) {
                ...
                // Error in API.
                $errorcount += 1;
                $type['global_error'] = xarML("Error updating item");
            }
        }
    } else {
