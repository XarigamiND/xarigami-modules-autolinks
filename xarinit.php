...
function autolinks_upgrade($oldversion)
{
    ...
    switch ($oldversion) {
        ...
        case '1.3':
            ...
            // experimental, and they will be wrapped by Xarigami functions in the future.
       ...
       case '1.7.0':
            //version update to reflect the need to update sample dd for 1.4.0 core
      case '1.7.1':
            // The current version.
...
function autolinks_delete()
{
    ...
    //if (!$result) {return;}
    //we want to remove dynamicdata itemtypes associated with autolinks

    $objectlist = xarModAPIFunc('dynamicdata','user','getobjects',array('moduleid'=>xarModGetIDFromName('autolinks')));

    foreach ($objectlist as $objectid=>$objectinfo)
    {
        if (!empty($objectid)) {
            $delobject = xarModAPIFunc('dynamicdata','admin','deleteobject',array('objectid' => $objectid,'moduleid'=>xarModGetIDFromName('autolinks')));
        }
